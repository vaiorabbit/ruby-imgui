...
module ImGuiDemo
  ...
  def self.AddFont(japanese_ttf_filepath = './jpfont/GenShinGothic-Normal.ttf', icon_ttf_filepath = './iconfont/fontawesome-webfont.ttf')
    ...
    config[:FontDataOwnedByAtlas] = true    # FontDataOwnedByAtlas = true;
    config[:ExtraSizeScale] = 1.0           # ExtraSizeScale = 1.0f;
    config[:GlyphMaxAdvanceX] = Float::MAX  # GlyphMaxAdvanceX = FLT_MAX;
    config[:RasterizerMultiply] = 1.0       # RasterizerMultiply = 1.0f;
    config[:RasterizerDensity] = 1.0        # RasterizerDensity = 1.0f;
...
module ImGuiDemo::ButtonAndCheckboxWindow
  ...
  def self.Show(is_open = nil)
    ...
    if ImGui::Button("Open/Close##Button")
      # ボタンがクリックされるとここにきます。
    ...
    ImGui::Checkbox("Open/Close##Checkbox", @@is_open)
...
module ImGuiDemo::ChildWindow
  ...
  def self.Show(is_open = nil)
    ...
    # "##"をつけることでラベル名は表示されません。
    ImGui::InputInt("##Line", @@line, 0, 0)
    goto_line |= ImGui::IsItemDeactivatedAfterEdit() # 入力欄で数字を入力してからEnterキーを押すか、入力欄からフォーカスが外れた時にgoto_lineはtrueになります。
    ...
    content_region_width = ImGui::GetContentRegionAvail()[:x]
